...
class SignalReceiver {
    ...
    struct SignalReceiverData {
        ...
        SignalReceiverData* ref() {
            assert(ref_count > 0);
            ref_count++;
            return this;
        }
        void unref(bool cleanup) {
            assert(ref_count > 0);
            ref_count--;

            if (cleanup && ref_count == 1) /* ensure nobody is iterating over the data */
            {
                sources.remove_if([](SignalSource& signal_source) -> bool { return signal_source.id == 0; });
            } else if (ref_count == 0)
                delete this;
        }
  ...
  public:
    template <class... Args, class CbFunction> uint64 connect(Signal<Args...>& signal, const CbFunction& callback) {
    ...
    uint64 connect(Signal<Args...>& signal, Instance* instance, const Method& method) {
        return SignalReceiver::connect(
            signal, [instance, method](Args&&... args) { (instance->*method)(std::forward<Args>(args)...); });
    }
    ...
    SignalReceiver() : signal_receiver_data(new SignalReceiverData()) {
    }
...
template <class... Args> class Signal : public SignalBase {
    ...
    struct Data {
        ...
        void unref(bool cleanup) {
            ...
            if (cleanup && ref_count == 1) /* ensure nobody is iterating over the data */
            {
                connections.remove_if([](Connection& conn) -> bool { return conn.id == 0; });
            } else if (ref_count == 0)
                delete this;
        }
  ...
  public:
    uint64 connect_impl(SignalReceiver* receiver, const CbFunction& callback) {
        assert(signal_data);
    ...
    void disconnect_impl(uint64 id) override {
        assert(signal_data);
