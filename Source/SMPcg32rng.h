...
class Pcg32Rng {
    ...
    static constexpr const uint64_t A =
        6364136223846793005ULL; // from C. E. Hayness, see TAOCP by D. E. Knuth, 3.3.4, table 1, line 26.
    static inline constexpr uint32_t ror32(const uint32_t bits, const uint32_t offset) {
        // bitwise rotate-right pattern recognized by gcc & clang iff 32==sizeof (bits)
        return (bits >> offset) | (bits << ((32 - offset) & 31));
    }
    static inline constexpr uint32_t pcg_xsh_rr(const uint64_t input) {
        // Section 6.3.1. 32-bit Output, 64-bit State: PCG-XSH-RR
        // http://www.pcg-random.org/pdf/toms-oneill-pcg-family-v1.02.pdf
        return (uint32_t)ror32((const uint32_t)((input ^ (input >> 18)) >> 27), input >> 59);
    }

  public:
    /// Initialize and seed from @a seed_sequence.
    template <class SeedSeq> explicit Pcg32Rng(SeedSeq& seed_sequence) : increment_(0), accu_(0) {
        seed(seed_sequence);
    }
    /// Initialize and seed by seeking to position @a offset within stream @a sequence.
    explicit Pcg32Rng(uint64_t offset, uint64_t sequence) : increment_(0), accu_(0) {
        seed(offset, sequence);
    }
    /// Initialize and seed the generator from a system specific nondeterministic random source.
    explicit Pcg32Rng() : increment_(0), accu_(0) {
        auto_seed();
    }
