...
void OutFile::write_file_type_and_version(const string& file_type, int file_version) {
    if (file) {
...
void OutFile::begin_section(const string& s) {
    file->put_byte('B'); // begin section
    write_raw_string(s);
}

void OutFile::end_section() {
...
void OutFile::write_float(const string& s, double f) {
    union {
        float f;
        int i;
    } u;
...
void OutFile::write_int(const string& s, int i) {
    file->put_byte('i'); // int
...
void OutFile::write_string(const string& s, const string& data) {
...
void OutFile::write_bool(const string& s, bool b) {
    file->put_byte('b'); // bool
    write_raw_string(s);
    file->put_byte(b ? 1 : 0);
}

void OutFile::write_float_block(const string& s, const vector<float>& fb) {
    file->put_byte('F');
...
void OutFile::write_operator(const string& name, const MorphOperatorPtr& op) {
    string op_name;

    if (op.get()) // (op == nullptr) => (op_name == "")
        op_name = op.get()->name();
