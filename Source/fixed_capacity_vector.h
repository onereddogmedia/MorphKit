...
#define FCV_ASSUME(EXPR) static_cast<void>((EXPR) ? void(0) : __builtin_unreachable())
...
#define FCV_REQUIRES_(...)                                                                                             \
    int FCV_CONCEPT_PP_CAT(_concept_requires_, __LINE__) = 42,                                                         \
...
#define FCV_REQUIRES(...)                                                                                              \
    template <int FCV_CONCEPT_PP_CAT(_concept_requires_, __LINE__) = 42,                                               \
              typename ::std::enable_if<(FCV_CONCEPT_PP_CAT(_concept_requires_, __LINE__) == 43) || (__VA_ARGS__),     \
                                        int>::type = 0> /**/
...
template <class = void> [[noreturn]] void assert_failure(char const* file, int line, char const* msg) {
...
///@{
template <typename T, typename... Args> static constexpr bool Constructible = is_constructible_v<T, Args...>;

template <typename T> static constexpr bool CopyConstructible = std::is_copy_constructible_v<T>;

template <typename T> static constexpr bool MoveConstructible = is_move_constructible_v<T>;

template <typename T, typename U> static constexpr bool Assignable = is_assignable_v<T, U>;

template <typename T>
static constexpr bool Movable = is_object_v<T> && Assignable<T&, T> && MoveConstructible<T> && is_swappable_v<T&>;

template <typename From, typename To> static constexpr bool Convertible = is_convertible_v<From, To>;

template <typename T> static constexpr bool Trivial = is_trivial_v<T>;

template <typename T> static constexpr bool Const = is_const_v<T>;

template <typename T> static constexpr bool Pointer = is_pointer_v<T>;
///@}  // Concepts

template <typename Rng> using range_iterator_t = decltype(begin(declval<Rng>()));

template <typename T> using iterator_reference_t = typename iterator_traits<T>::reference;

template <typename T> using iterator_category_t = typename iterator_traits<T>::iterator_category;

template <typename T, typename Cat, typename = void> struct Iterator_ : false_type {};

template <typename T, typename Cat>
struct Iterator_<T, Cat, void_t<iterator_category_t<T>>> : bool_<Convertible<iterator_category_t<T>, Cat>> {};

/// \name Concepts (poor-man emulation using type traits)
///@{
template <typename T> static constexpr bool InputIterator = Iterator_<T, input_iterator_tag>{};

template <typename T> static constexpr bool ForwardIterator = Iterator_<T, forward_iterator_tag>{};

template <typename T> static constexpr bool OutputIterator = Iterator_<T, output_iterator_tag>{} || ForwardIterator<T>;

template <typename T> static constexpr bool BidirectionalIterator = Iterator_<T, bidirectional_iterator_tag>{};

template <typename T> static constexpr bool RandomAccessIterator = Iterator_<T, random_access_iterator_tag>{};

template <typename T> static constexpr bool RandomAccessRange = RandomAccessIterator<range_iterator_t<T>>;
///@}  // Concepts
...
template <typename Rng, typename Index, FCV_REQUIRES_(RandomAccessRange<Rng>)>
...
template <typename ForwardIt, FCV_REQUIRES_(ForwardIterator<ForwardIt>)>
...
template <typename InputIt, typename OutputIt, FCV_REQUIRES_(InputIterator<InputIt>&& OutputIterator<OutputIt>)>
...
template <class BinaryPredicate, class InputIterator1, class InputIterator2,
          FCV_REQUIRES_(InputIterator<InputIterator1>&& InputIterator<InputIterator2>)>
...
template <typename T> struct zero_sized {
    ...
    template <typename... Args, FCV_REQUIRES_(Constructible<T, Args...>)>
    static constexpr void emplace_back(Args&&...) noexcept {
        FCV_EXPECT(false && "tried to emplace_back on empty storage");
    }
    /// Removes the last element of the storage.
    /// Always fails for empty storage.
    static constexpr void pop_back() noexcept {
        FCV_EXPECT(false && "tried to pop_back on empty storage");
    }
    ...
    static constexpr void unsafe_set_size(size_t new_size) noexcept {
        FCV_EXPECT(new_size == 0 && "tried to change size of empty storage to "
                                    "non-zero value");
    }
    ...
    template <typename InputIt, FCV_REQUIRES_(InputIterator<InputIt>)>
    ...
    /// zero elements.
    template <typename U, FCV_REQUIRES_(Convertible<U, T>)> constexpr zero_sized(initializer_list<U> il) noexcept {
        FCV_EXPECT(il.size() == 0 && "tried to construct storage::empty from a "
                                     "non-empty initializer list");
    }
};

/// Storage for trivial types.
template <typename T, size_t Capacity> struct trivial {
  ...
  private:
    ...
    // non-const elements:
    using data_t = conditional_t<!Const<T>, array<T, Capacity>, const array<remove_const_t<T>, Capacity>>;
  ...
  public:
    ...
    template <typename... Args, FCV_REQUIRES_(Constructible<T, Args...>and Assignable<value_type&, T>)>
    constexpr void emplace_back(Args&&... args) noexcept {
        FCV_EXPECT(!full() && "tried to emplace_back on full storage!");
    ...
    constexpr void pop_back() noexcept {
        FCV_EXPECT(!empty() && "tried to pop_back from empty storage!");
        unsafe_set_size(size() - 1);
    }
    ...
    constexpr void unsafe_set_size(size_t new_size) noexcept {
        FCV_EXPECT(new_size <= Capacity && "new_size out-of-bounds [0, Capacity]");
  ...
  private:
    ...
    static constexpr array<remove_const_t<T>, Capacity> unsafe_recast_init_list(initializer_list<U>& il) noexcept {
        FCV_EXPECT(il.size() <= capacity() && "trying to construct storage from an "
                                              "initializer_list "
  ...
  public:
    ...
    template <typename U, FCV_REQUIRES_(Convertible<U, T>)>
    constexpr trivial(initializer_list<U> il) noexcept : data_(unsafe_recast_init_list(il)) {
        unsafe_set_size(static_cast<size_type>(il.size()));
    }
};

/// Storage for non-trivial elements.
template <typename T, size_t Capacity> struct non_trivial {
  ...
  private:
    ...
    using aligned_storage_t = aligned_storage_t<sizeof(remove_const_t<T>), alignof(remove_const_t<T>)>;
    using data_t = conditional_t<!Const<T>, aligned_storage_t, const aligned_storage_t>;
  ...
  public:
    ...
    /// Contract: the storage is not full.
    template <typename... Args, FCV_REQUIRES_(Constructible<T, Args...>)>
    void emplace_back(Args&&... args) noexcept(noexcept(new(end()) T(forward<Args>(args)...))) {
        FCV_EXPECT(!full() && "tried to emplace_back on full storage");
    ...
    /// \warning: The size of the storage is not changed.
    template <typename InputIt, FCV_REQUIRES_(InputIterator<InputIt>)>
    void unsafe_destroy(InputIt first, InputIt last) noexcept(is_nothrow_destructible_v<T>) {
        FCV_EXPECT(first >= data() && first <= end() && "first is out-of-bounds");
        FCV_EXPECT(last >= data() && last <= end() && "last is out-of-bounds");
    ...
    /// Contract: `il.size() <= capacity()`.
    template <typename U, FCV_REQUIRES_(Convertible<U, T>)>
    constexpr non_trivial(initializer_list<U> il) noexcept(noexcept(emplace_back(index(il, 0)))) {
        FCV_EXPECT(il.size() <= capacity() && "trying to construct storage from an "
                                              "initializer_list "
                                              "whose size exceeds the storage capacity");
...
using _t = conditional_t<Capacity == 0, zero_sized<T>,
                         conditional_t<Trivial<T>, trivial<T, Capacity>, non_trivial<T, Capacity>>>;
...
template <typename T, size_t Capacity> struct fixed_capacity_vector : private fcv_detail::storage::_t<T, Capacity> {
  private:
  ...
  private:
    ...
    template <typename It> constexpr void assert_iterator_in_range(It it) noexcept {
        static_assert(fcv_detail::Pointer<It>);
        FCV_EXPECT(begin() <= it && "iterator not in range");
        FCV_EXPECT(it <= end() && "iterator not in range");
    }

    template <typename It0, typename It1> constexpr void assert_valid_iterator_pair(It0 first, It1 last) noexcept {
        static_assert(fcv_detail::Pointer<It0>);
        static_assert(fcv_detail::Pointer<It1>);
        FCV_EXPECT(first <= last && "invalid iterator pair");
    }

    template <typename It0, typename It1> constexpr void assert_iterator_pair_in_range(It0 first, It1 last) noexcept {
  ...
  public:
    ...
    template <typename U, FCV_REQUIRES_(fcv_detail::Constructible<T, U>&& fcv_detail::Assignable<reference, U&&>)>
    ...
    FCV_REQUIRES(fcv_detail::Constructible<T, T>&& fcv_detail::Assignable<reference, T&&>)
    void push_back() noexcept(noexcept(emplace_back(T{}))) {
        FCV_EXPECT(!full() && "vector is full!");
        emplace_back(T{});
    }

    template <typename... Args, FCV_REQUIRES_(fcv_detail::Constructible<T, Args...>)>
    constexpr iterator
    emplace(const_iterator position,
            Args&&... args) noexcept(noexcept(move_insert(position, declval<value_type*>(), declval<value_type*>()))) {
        FCV_EXPECT(!full() && "tried emplace on full fixed_capacity_vector!");
    ...
    constexpr iterator insert(const_iterator position,
        ...
        FCV_EXPECT(!full() && "tried insert on full fixed_capacity_vector!");
        assert_iterator_in_range(position);
    ...
    constexpr iterator insert(const_iterator position, size_type n, const T& x) noexcept(noexcept(push_back(x))) {
        ...
        FCV_EXPECT(new_size <= capacity() && "trying to insert beyond capacity!");
        auto b = end();
        while (n != 0) {
            push_back(x);
            --n;
    ...
    template <class InputIt, FCV_REQUIRES_(fcv_detail::InputIterator<InputIt>and fcv_detail::Constructible<
                                           value_type, fcv_detail::iterator_reference_t<InputIt>>)>
    constexpr iterator insert(const_iterator position, InputIt first,
        ...
        if constexpr (fcv_detail::RandomAccessIterator<InputIt>) {
            FCV_EXPECT(size() + static_cast<size_type>(last - first) <= capacity() &&
    ...
    template <class InputIt, FCV_REQUIRES_(fcv_detail::InputIterator<InputIt>)>
    ...
    constexpr iterator erase(const_iterator first, const_iterator last) noexcept {
        ...
        if (first != last) {
            unsafe_destroy(fcv_detail::move(p + (last - first), end(), p), end());
    ...
    constexpr void swap(fixed_capacity_vector& other) noexcept(is_nothrow_swappable_v<T>) {
    ...
    constexpr void resize(size_type sz, T const& value) noexcept(is_nothrow_copy_constructible_v<T>) {
        ...
        if (sz > size()) {
            FCV_EXPECT(sz <= capacity() && "fixed_capacity_vector cannot be resized to "
  ...
  private:
    FCV_REQUIRES(fcv_detail::MoveConstructible<T> or fcv_detail::CopyConstructible<T>)
    constexpr void
    emplace_n(size_type n) noexcept((fcv_detail::MoveConstructible<T> && is_nothrow_move_constructible_v<T>) ||
                                    (fcv_detail::CopyConstructible<T> && is_nothrow_copy_constructible_v<T>)) {
        FCV_EXPECT(n <= capacity() && "fixed_capacity_vector cannot be "
  ...
  public:
    ...
    FCV_REQUIRES(fcv_detail::Movable<value_type>)
    constexpr void
    resize(size_type sz) noexcept((fcv_detail::MoveConstructible<T> && is_nothrow_move_constructible_v<T>) ||
                                  (fcv_detail::CopyConstructible<T> && is_nothrow_copy_constructible_v<T>)) {
        if (sz == size()) {
            return;
        }
    ...
    constexpr fixed_capacity_vector(fixed_capacity_vector const& other) noexcept(noexcept(insert(begin(), other.begin(),
                                                                                                 other.end()))) {
    ...
    constexpr fixed_capacity_vector(fixed_capacity_vector&& other) noexcept(noexcept(move_insert(begin(), other.begin(),
                                                                                                 other.end()))) {
        // nothin to assert: size of other cannot exceed capacity
        // because both vectors have the same type
    ...
    constexpr fixed_capacity_vector& operator=(fixed_capacity_vector const& other) noexcept(
    ...
    constexpr fixed_capacity_vector& operator=(fixed_capacity_vector&& other) noexcept(
    ...
    FCV_REQUIRES(fcv_detail::CopyConstructible<T> or fcv_detail::MoveConstructible<T>)
    explicit constexpr fixed_capacity_vector(size_type n) noexcept(noexcept(emplace_n(n))) {
    ...
    FCV_REQUIRES(fcv_detail::CopyConstructible<T>)
    constexpr fixed_capacity_vector(size_type n, T const& value) noexcept(noexcept(insert(begin(), n, value))) {
        FCV_EXPECT(n <= capacity() && "size exceeds capacity");
    ...
    constexpr fixed_capacity_vector(InputIt first, InputIt last) {
        if constexpr (fcv_detail::RandomAccessIterator<InputIt>) {
            ...
            FCV_EXPECT(static_cast<size_type>(last - first) <= capacity() && "range size exceeds capacity");
        }
        insert(begin(), first, last);
    }

    template <typename U, FCV_REQUIRES_(fcv_detail::Convertible<U, value_type>)>
    constexpr fixed_capacity_vector(initializer_list<U> il) noexcept(noexcept(base_t(move(il))))
    ...
    constexpr void assign(initializer_list<T> const& il) {
        FCV_EXPECT(il.size() <= capacity() && "initializer_list size exceeds capacity");
        clear();
...
template <typename T, size_t Capacity>
constexpr bool operator==(fixed_capacity_vector<T, Capacity> const& a,
                          fixed_capacity_vector<T, Capacity> const& b) noexcept {
    return a.size() == b.size() and fcv_detail::cmp(a.begin(), a.end(), b.begin(), b.end(), equal_to<>{});
}

template <typename T, size_t Capacity>
constexpr bool operator<(fixed_capacity_vector<T, Capacity> const& a,
                         fixed_capacity_vector<T, Capacity> const& b) noexcept {
    return fcv_detail::cmp(a.begin(), a.end(), b.begin(), b.end(), less<>{});
}
...
constexpr bool operator<=(fixed_capacity_vector<T, Capacity> const& a,
                          fixed_capacity_vector<T, Capacity> const& b) noexcept {
    return fcv_detail::cmp(a.begin(), a.end(), b.begin(), b.end(), less_equal<>{});
}

template <typename T, size_t Capacity>
constexpr bool operator>(fixed_capacity_vector<T, Capacity> const& a,
                         fixed_capacity_vector<T, Capacity> const& b) noexcept {
    return fcv_detail::cmp(a.begin(), a.end(), b.begin(), b.end(), greater<>{});
}

template <typename T, size_t Capacity>
constexpr bool operator>=(fixed_capacity_vector<T, Capacity> const& a,
                          fixed_capacity_vector<T, Capacity> const& b) noexcept {
    return fcv_detail::cmp(a.begin(), a.end(), b.begin(), b.end(), greater_equal<>{});
}
