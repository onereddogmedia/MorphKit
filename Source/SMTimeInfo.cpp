...
TimeInfoGenerator::TimeInfoGenerator(double mix_freq) : m_mix_freq(mix_freq) {
}

double TimeInfoGenerator::samples_to_beats(double samples) const {
    return samples / m_mix_freq * (m_tempo / 60);
}

double TimeInfoGenerator::samples_to_ms(double samples) const {
    return samples / m_mix_freq * 1000;
}

void TimeInfoGenerator::start_block(uint64 audio_time_stamp, uint n_samples, double ppq_pos, double tempo) {
...
void TimeInfoGenerator::update_time_stamp(uint64 audio_time_stamp) {
    m_position = (uint)(audio_time_stamp - m_audio_time_stamp);
}

TimeInfo TimeInfoGenerator::time_info(double offset_ms) const {
    TimeInfo ti;
