...
double AudioTool::Block2Energy::energy(const AudioBlock& block) {
...
double AudioTool::compute_energy(const Audio& audio) {
    double e = 0;
    double e_norm = 0;
    ...
    if (audio.loop_type == Audio::LoopType::LOOP_FRAME_FORWARD ||
        ...
        start = sm_bound<size_t>(start, (size_t)audio.loop_start, end);

        // last loop block is also used for normalization => (loop_end + 1)
        end = sm_bound<size_t>(start, (size_t)audio.loop_end + 1, end);
    }
...
void AudioTool::normalize_factor(double norm, Audio& audio) {
    ...
    audio.original_samples_norm_db = (float)db_from_factor(samples_factor * norm, -200);
}

void AudioTool::normalize_energy(double energy, Audio& audio) {
...
bool AudioTool::get_auto_tune_factor(Audio& audio, double& tune_factor) {
...
void AudioTool::apply_auto_tune_factor(Audio& audio, double tune_factor) {
    for (size_t f = 0; f < audio.contents.size(); f++) {
...
void AudioTool::auto_tune_smooth(Audio& audio, int partials, double smooth_ms, double smooth_percent) {
