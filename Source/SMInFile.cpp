 ...
 */
InFile::InFile(const string& filename) : file_delete(true) {
 ...
 *
 * \param file_ the input stream object to read data from
 */
InFile::InFile(GenericIn* file_) : file(file_), file_delete(false) {
    current_event = NONE;
    read_file_type_and_version();
}
...
void InFile::read_file_type_and_version() {
    if (file) {
        if (file->get_byte() == 'T')
            ...
            if (read_raw_int(blob_size)) {
                ...
                if (read_raw_string(blob_sum)) {
                    if (blob_size == -1) {
                        ...
                        size_t blob_pos = file->get_pos();
                        if (file->skip((size_t)blob_size)) // skip actual blob data
                        {
                            current_event = BLOB;
                            current_event_blob_size = (size_t)blob_size;
...
bool InFile::read_raw_int(int& i) {
    if (file->read(&i, 4) == 4) {
        return true;
    } else {
        return false;
...
bool InFile::read_raw_bool(bool& b) {
    char bchar;
...
bool InFile::read_raw_float(float& f) {
    union {
        float f;
        int i;
    } u;
...
bool InFile::read_raw_float_block(vector<float>& fb) {
    int size;
...
bool InFile::read_raw_uint16_block(vector<uint16_t>& ib) {
    int size;
    if (!read_raw_int(size))
        return false;
 ...
 * is responsible for freeing the GenericIn object when done. nullptr will be returned on
 * error.
 */
 ...
 */
const std::vector<uint16_t>& InFile::event_uint16_block() {
