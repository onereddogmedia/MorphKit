...
void MorphOperatorModule::reset_value(const TimeInfo&) {
}

void MorphOperatorModule::set_notify_value(float value) {
    m_notify_value = value;
    m_have_notify_value = true;
}

bool MorphOperatorModule::get_notify_value(float& value) {
...
void MorphOperatorModule::update_shared_state(const TimeInfo&) {
}

MorphModuleSharedState* MorphOperatorModule::create_shared_state() {
    return nullptr;
}

void MorphOperatorModule::set_shared_state(MorphModuleSharedState* /*new_shared_state*/) {
}

SpectMorph::Random* MorphOperatorModule::random_gen() const {
    return morph_plan_voice->morph_plan_synth()->random_gen();
}

RTMemoryArea* MorphOperatorModule::rt_memory_area() const {
    MorphOutputModule* output = morph_plan_voice->output();
    return output->rt_memory_area();
}

TimeInfo MorphOperatorModule::time_info() const {
    MorphOutputModule* output = morph_plan_voice->output();
    TimeInfo time;
...
float MorphOperatorModule::apply_modulation(const ModulationData& mod_data) const {
    double base;
    double value = 0;
    ...
    if (mod_data.main_control_type == MorphOperator::CONTROL_GUI) {
        ...
        if (mod_data.main_control_type == MorphOperator::CONTROL_OP) {
            ...
            value = (morph_plan_voice->control_input(/* gui */ 0, mod_data.main_control_type,
                                                     /* mod (not used) */ nullptr) +
                     1) *
                    0.5;
        }
    }
    ...
    for (const auto& entry : mod_data.entries) {
        ...
        if (entry.control_type == MorphOperator::CONTROL_OP)
            ...
            mod_value = morph_plan_voice->control_input(/* gui (not used) */ 0, entry.control_type,
                                                        /* mod (not used) */ nullptr);
    ...
    switch (mod_data.property_scale) {
        case Property::Scale::LOG:
            value = base * exp2f(mod_data.value_scale * (float)value);
            break;
        default:
        case Property::Scale::LINEAR:
        case Property::Scale::NONE:
            value = base + mod_data.value_scale * value;
    }
    return sm_clamp<float>((float)value, mod_data.min_value, mod_data.max_value);
}
