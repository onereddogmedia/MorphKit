...
class IFFTSynth {
    ...
    FFT::PlanMap fftsr_destructive_float_plan;

    enum { SIN_TABLE_SIZE = 4096, SIN_TABLE_MASK = 4095 };
...
inline void IFFTSynth::render_partial(double mf_freq, double mag, double phase) {
    const int range = 4;
    ...
    const float nmag = (const float)(mag * mag_norm);

    // rotation for initial phase; scaling for magnitude

    /* the following block computes sincos (phase + phase_adjust) */
    int iarg = sm_round_positive(phase * ((double)SIN_TABLE_SIZE / (2 * M_PI)));
    ...
    if (ibin > range && 2 * (ibin + range) < static_cast<int>(block_size)) {
        ...
        for (int i = -range; i <= range; i++) {
            ...
            if ((ibin + i) < 0) {
                ...
                int p = static_cast<int>(block_size) - (2 * (ibin + i) - static_cast<int>(block_size));
